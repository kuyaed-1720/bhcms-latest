...
<template>
	<AppLayout title="Appointment">
		...
		<div class="py-6">
			<div class="w-auto mx-8 mt-4 px-6 py-4 bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
				<div class="overflow-hidden sm:rounded-lg">
					<div class="p-6">
						<h1 class="mt-2 text-2xl font-medium text-gray-900 dark:text-white">Welcome to BHCMS Appointments Section!</h1>

						<p class="mt-2 text-gray-600 dark:text-gray-400 leading-relaxed`border-b border-gray-900">
							BHCMS Appointments is a system features that provides easier way to schedule appointments in the field.
						</p>
					</div>
				</div>
			</div>
		...
		<div class="w-auto mx-8 flex items-center justify-between pl-4 py-2">
			<Link :href="route('appointments.create')" class="text-center flex items-center justify-center text-gray-500 w-48 h-8 dark:text-gray-300 dark:bg-orange-600 hover:underline">
				Create Appointment
			</Link>
			<div class="w-6/12 flex flex-row justify-between">
				<Link
					:href="route('appointments', { ...filters, status: '' })"
				...
				<Link
					:href="route('appointments', { ...filters, status: 'pending' })"
				...
				<Link
					:href="route('appointments', { ...filters, status: 'cancelled' })"
				...
				<Link
					:href="route('appointments', { ...filters, status: 'completed' })"
		...
		<div class="relative flex flex-col w-auto overflow-hidden mx-8 mt-2 bg-white dark:bg-gray-200 shadow-xl">
			<table class="w-full text-left table-auto">
				<thead class="bg-gray-900 dark:bg-white border-b border-gray-900 dark:border-gray-300 text-gray-200 dark:text-gray-900">
					<tr>
						...
						<th class="p-4 w-24 text-sm font-normal leading-none" v-if="!filters.status || filters.status === 'pending'">ACTION</th>
					</tr>
				</thead>
				<tbody>
					<tr class="hover:bg-gray-300" v-for="appointment in appointments" :key="appointment.id">
						...
						<!-- Limited width -->
						<td class="p-4 text-sm text-gray-900 dark:text-gray-700">{{ appointment.appointment_date }} {{ appointment.appointment_time }}</td>
						<td class="p-4 text-sm text-gray-900 dark:text-gray-700" v-if="!filters.status">{{ appointment.status }}</td>
						<td class="p-4 flex flex-row justify-between" v-if="!filters.status || filters.status === 'pending'">
